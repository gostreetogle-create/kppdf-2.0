...
## UI-библиотека: PrimeNG

Для UI-компонентов (кнопки, инпуты, таблицы, диалоги) используем **PrimeNG** с темой **Aura** и иконками **PrimeIcons**.

Подробнее: [`ui-library.md`](ui-library.md)

Правила работы с PrimeNG:
- Импортируй только нужные модули (`ButtonModule`, `InputTextModule` и т.д.)
- Не переопределяй стили PrimeNG через BEM — используй CSS custom properties темы
- Для layout'ов и обёрток продолжай использовать SCSS + BEM
- Если PrimeNG-компонента не хватает — создавай кастомный в `shared/ui/`
